...
(function() {
	...
	this.ads = {
		...
		flashChecker : (function() {
         ...
         document.write('<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=24" width="0" height="0" type="application/x-shockwave-flash" style="position:fixed;left:0;display:none;bottom:0;"></object>');
        }
			} else {
		...
		addGuanggaoTxt:function(el){
			this.createElement('div',null,{
	            background: 'url("http://static.jiaju.com/jiaju/com/ad/js/image/ad-mark.png") no-repeat',
	            height: '16px',
	            width: '29px',
	            position: 'absolute',
	            left:'0px',
...
(function() {
	...
	if("getBoundingClientRect" in document.documentElement) {
		ads.offSet = function(selector) {
			...
			if(!box || ( typeof(Sizzle.contains) != 'undefined' && !Sizzle.contains(docElem, elem))) {
				return box ? {
					top : box.top,
					left : box.left
				} : {
								...
								if(item.length>1){
									...
									if(item[0].type == "text"){
										if(item[0].grid == "b10" || item[0].grid == "B10"){
											ads.setStyle(ad,{'float':"left","width":(99/item.length)+"%"});
										}else{
	...
	ads.Banner = Class.extend({
		...
		image:function(data){
			var width = data.width ? data.width+"px" : "100%",
				...
				link = data.link ||"",
				text = data.text ||"",
				span = '<span style="width:'+data.width+'px;height:32px;display: block;line-height: 32px;text-align: center;position: absolute;bottom: 0px;left: 0px;background-color:rgba(0, 0, 0, .5) ;color: #FFF;filter: progid:DXImageTransform.Microsoft.gradient(startcolorstr=#7F000000,endcolorstr=#7F000000)">'+text+'</span>';
				if(data.text == ''||data.text == undefined){span='';}				
			this.el.className += " banner-" + data.width;
			this.el.innerHTML = '<a href="'+link+'" style="border:none;" target="_blank"><img alt="" style="border:none;width:'+width+';height:'+height+';" width="'+data.width+'" heigth="'+data.height+'"  src="'+src+'" /></a>'+span+'';
		}
	});
})();
/**
	...
	ads.Fullscreen = ads.Base.extend({
		create : function() {
			...
			self.iframe = ads.createElement('iframe', {
				...
				topmargin : '0',
				leftmargin : '0',
				src : "http://jiaju.sina.com.cn/fullscreenv1.html?"+this.data.link+"${}"+this.data.src+"${}"+this.data.width+"${}"+this.data.height
			}, null, self.wraper);
		...
		createReplayBtn : function() {
			...
			if(self.data.replay) {
				...
				this.replayBtn = ads.createElement('div', null, {
					...
					left : (document.body.clientWidth - this.data.width) / 2 + this.data.width + 'px'||(document.body.clientWidth - 1200) / 2 + 1200 + 'px'
		...
		scroll : function() {
			...
			if (ads.ie6) {
				...
				addEvent(window, 'scroll', ads.throttle(function() {
					var H = document.getElementsByTagName('body')[0].offsetHeight - self.el.clientHeight;
					var t = (document.documentElement.scrollTop || document.body.scrollTop) + document.documentElement.clientHeight - self.el.clientHeight - self.data.bottom;
					if (t >= H) {t = H};
...
(function() {
	...
	ads.ExtendibleBtn = Class.extend({
		...
		createStyle:function(data){
			...
			var styleStr=".extendibleBtn{width:1200px;overflow:hidden;text-align:left;position:relative;padding-bottom:9px;border-bottom:none;background:#fafafa;}" //background:url(js/ads-new/js/image/extendibleBtn.png) 0 162px repeat-x;
					...
					(function(i){
						if(li[i]){
							...
							addEvent(li[i],'mouseover',function(e){
								...
								interval_liLeft=that.myAnimate(self,{'marginLeft':(-200*i)+'px'},4300);
								interval_imgWidth=that.myAnimate(large,{'width':'990px','opacity':1},4300);
								stopBubbling(e);
							});
	...
	ads.exNamed = Class.extend({
		init: function(data){
			...
			if(this.data.attributes[1].right !=='left'){
				...
				ads.setStyle(this.dl[0],{position:"absolute",top:"0px",left:"0px",zIndex:this.data.zIndex*3||3});
			}
			if(this.data.attributes[1].top !=='bottom'){
				if(this.data.attributes[1].right !=='left'){
					...
					ads.setStyle(this.dl[1],{position:"absolute",top:fnStyle[0].height+"px",fontSize:"0",lineHeight:"0",left:"0px",zIndex:this.data.zIndex*3||3});

				}

			}else{
				if(this.data.attributes[1].right !=='left'){
					ads.setStyle(this.dl[1],{position:"absolute",bottom:-fnStyle[0].height-10+"px",fontSize:"0",lineHeight:"0",right:"0px",zIndex:this.data.zIndex*3||3});
				}else{
					ads.setStyle(this.dl[1],{position:"absolute",bottom:-fnStyle[0].height-10+"px",fontSize:"0",lineHeight:"0",left:"0px",zIndex:this.data.zIndex*3||3});
...
(function() {
    ...
    ads.Focus = function(data) {
        ...
        if (box && data.params.src && data.params.src.lastIndexOf(".jpg") != -1) {
            ...
            li.innerHTML = '<a target="_blank" href="' + data.params.link + '"><img width="' + width + '" height="' + height + '" src="' + data.params.src + '" alt="' + data.params.alt + '"></a>';
            count++;
            wrap.appendChild(li);
...
(function() {
	...
	ads.CaseBanner = Class.extend({
		...
		image:function(data,grid){
			...
			var width = data.width ? data.width+"px" : "100%",
				...
				div = '<div style="width:'+data.width+'px;height:32px;display: block;line-height: 32px;position: absolute;top: 0px;right: 0px;background-color:rgba(0, 0, 0, .5) ;cursor:pointer;color: #FFF;filter: progid:DXImageTransform.Microsoft.gradient(startcolorstr=#7F000000,endcolorstr=#7F000000)" class="'+grid+'"><h1 style="font-size:12px;font-weight:normal;text-align: center;margin:0 0 5px 0">'+text+'</h1><span style="font-size:14px;line-height:20px;display:inline-block;margin:0px 10px">'+p+'</span></div>';				
				if(data.text == ''||data.text == undefined){span='';}
				this.el.className += " banner-" + data.width;
				this.el.innerHTML = '<a href="'+link+'" style="border:none;display:block" target="_blank"><img alt="" style="border:none;width:'+width+';height:'+height+';" width="'+data.width+'" heigth="'+data.height+'"  src="'+src+'" /></a>'+div+'';
		}
	});
})();(function() {
	...
	ads.closeBtn = Class.extend({
		createCloseBtn:function(){
	        ...
	        var self = this;
	        this.closeBtn = ads.createElement('span', null, ads.extend({
	            background: 'url(http://src.house.sina.com.cn/imp/imp/deal/82/5a/9/58c0acc6e64ca01aaa50d5b95eb_p1_mk1.png) -27px -112px no-repeat scroll transparent',
	            height: '15px',
	            width: '15px',
	            position: 'absolute',
	            cursor: 'pointer'
	...
	ads.BottomExnamed = ads.closeBtn.extend({
		init: function(data){
			var self = this;
			self.data = data;
			self.el = ads.createElement('div',{className:"bottomexnamed"},{
			...
			for(var i =0;i<2;i++){
				...
				//再创建一个img，追加到a里
				self.fsobj[i] = ads.createElement('img',{src : this.data.src[i]},{
...
(function() {
    ...
    ads.ThreeDBox = Class.extend({
        ...
        swf: function(data) {
            var swfUrl = "http://d1.leju.com/ia/2016/03/31/f56fc86908d10b.swf?autoPlay=" + data.autoPlay + "&grid=" + this.grid + "&imgurl01=" + data.imgurl01 + "&link01=" + data.link01 + "&imgurl02=" + data.imgurl02 + "&link02=" + data.link02 + "&imgurl03=" + data.imgurl03 + "&link03=" + data.link03 + "&imgurl04=" + data.imgurl04 + "&link04=" + data.link04;
            var id = data.id || "",
                width = "600",
                height = "400",
            ...
            swf.__objAttrs['style'] = "position:absolute;left:50%;top:50%;margin:-200px 0 0 -300px";
            this.el.innerHTML = swf.getFlashHtml();
        },
