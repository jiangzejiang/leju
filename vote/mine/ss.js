    ...
    function renderData(report_list, myAllData) {
        ...
        for (var i = 0; i < report_list.length; i++) {
            if (report_list[i].oid == oid) {
                ...
                left_button.on('click', leftClick);

                function leftClick() {
                    left_button.off('click');
                    // console.log(data)
                    // 如果不为空
                    var succeedTime = localStorage.getItem('succeedTime');
                    var date = new Date()
                    var nowTime = date.getTime();
                    // 后台设置的时间 * 1000
                    var frequency = parseInt(myAllData.data.refresh_frequency) * 1000;
                     // 如果并非第一次投票，或者距离上一次投票时间小于后台配置的时间
                    if (succeedTime && nowTime < (parseInt(succeedTime) + frequency)) {
                        var pastTiem = parseInt(frequency - parseInt(nowTime - succeedTime));
                        if (pastTiem < 0) {
                            alert('请勿频繁投票');
                        } else {
                            alert('请勿频繁投票，请' + formatSeconds(pastTiem / 1000) + '后再试');
                        }
                        left_button.on('click', leftClick)
                    } else {
                        // 如果第一次投票，或者距离上一次投票时间已经达到了后台配置的时间
                        $.getJSON(votePort + 'qid' + data.qid + "&" + 'qid_value=' + data.oid + "&callback=?", function (data) {
                            if (data.msg) {
                                ...
                                if (data.msg == '投票成功') {
                                    ...
                                    // console.log('localStorageTime' + localStorage.getItem('succeedTime'));
                                    // console.log('目前时间' + milliseconds);
                                    // console.log('passTime：' + pastTiem);
                                    // console.log(formatSeconds(pastTiem))
                                    if (pastTiem < 0) {
                                        alert('请勿频繁投票');
                                    } else {
