...
//获取投票总数
var allCountPort = 'http://api.survey.leju.com/index.php?c=interface&a=get_survey&sur_id=2156&type=result&callback=?'
// 获取人物所有信息
var allPersonPort = 'http://zx.jiaju.sina.com.cn/index.php?app=Api&mod=Survey&sur_id=2156&callback=?'
// 投票接口
var votePort = 'http://zx.jiaju.sina.com.cn/index.php?app=Api&mod=Survey&act=subAttr&sur_id=2156&qid_key='

var xiangqingUrl = 'http://sh.jiaju.sina.com.cn/6427339243571718353/2018/0724/6427346277356604149.html'
$(function () {
    ...
    var obj = {
        ...
        getPersonData: function () {
            $.getJSON(("http://zx.jiaju.sina.com.cn/index.php?app=Api&mod=Survey&sur_id=2156&callback=?", function (data) {
                this.option = data.data.question[0].option;
                searchInput(option);
                this.renderData();
            }))
        },
        ...
        renderData: function () {
            ...
            $.getJSON(allCountPort, function (myCount) {
                ...
                if (countJson == newCountJosn) {
                    ...
                    report_list.forEach(function (item) {
                        index++;
                        for (var countItem in myCountData) {
                            if (countItem.toString() === item.oid.toString()) {
                                item.count = myCountData[item.oid];
                            }
                        }
                        if (index == report_list.length) {
                            this.doFunction(report_list);
                        }
                    })
